...
export default function GcrPage() {
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const overallGcr = totalBilled
      ? ((totalPaid / totalBilled) * 100).toFixed(2)
      : 0;

    return { overallGcr, totalPaid, totalBilled };
  ...
  const filteredMonthlyData = useMemo(() => {
    ...
    return monthsToShow.map((month) => {
      ...
      const actual = vals && vals.billed > 0
  ...
  const avgVsLastMonthData = useMemo(() => {
    ...
    const lastMonth = dayjs(filtered[filtered.length - 1].date).format("MMM");

    const last3 = data.slice(-3);
    const avg3Paid = last3.reduce((sum, d) => sum + d.paid, 0);
    const avg3Billed = last3.reduce((sum, d) => sum + d.billed, 0);
    const avg3 = avg3Billed > 0 ? ((avg3Paid / avg3Billed) * 100).toFixed(2) : 0;

    const lastMonthData = filteredMonthlyData.find((m) => m.month === lastMonth);

    return [
      { label: "Last 3 Months Avg", gcr: +avg3 },
      { label: "Last Month", gcr: lastMonthData ? lastMonthData.actual : 0 },
    ];
  }, [data, selectedClient, selectedMonth, filteredMonthlyData]);
